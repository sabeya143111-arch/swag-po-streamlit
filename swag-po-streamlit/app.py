# app.py  (Streamlit + Odoo PO Creator + Multi-company)
...
def load_companies(models, db, uid, password):
    """Allowed companies list (id, name)."""
...
if st.button("🚀 Create Draft Purchase Order"):
    ...
    if not companies:
        st.error("Koi company nahi mili; Odoo me multi-company rights check karo.")
        st.stop()

    company_names = [c["name"] for c in companies]
    selected_name = st.selectbox("Select Company (PO yahi company me banega)", company_names)

    # user selects then clicks again? simple guard:
    if not selected_name:
        st.error("Company select karo.")
        st.stop()

    selected_company = next(c for c in companies if c["name"] == selected_name)
    company_id = selected_company["id"]

    # XML-RPC context for correct company
    ctx = {"allowed_company_ids": [company_id], "company_id": company_id}

    st.write(f"Working company: **{selected_name}** (ID {company_id})")
    ...
    po_vals = {
        ...
        "date_order": po_date,
        "company_id": company_id,   # safety
        "order_line": order_lines,
    }

    # --- Create Purchase Order in selected company ---
    try:
    ...
    st.success(f"✅ Draft Purchase Order created in {selected_name}: ID {po_id}")
    st.info(
        "Next steps Odoo me:\n"
